                ...
                <div class="max-w-4xl space-y-6">
                    <div class="rounded-3xl bg-white p-8 shadow-sm ring-1 ring-slate-200/60">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                <h1 class="text-3xl font-semibold text-slate-900">Edit Subscription Plan</h1>
                                <p class="mt-2 text-sm text-slate-500">Update plan details, pricing, and status.</p>
                            </div>
                        ...
                        <form action="{{ route('admin.subscription_plans.update', $subscriptionPlan) }}" method="POST" class="mt-6 space-y-6">
                            @csrf
                            @method('PUT')
                            @include('admin.subscription_plans._form', ['subscriptionPlan' => $subscriptionPlan])

                            <button type="submit" class="rounded-2xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white hover:bg-slate-800">Save changes</button>
                        </form>
                    </div>
                </div>
            </main>
